...
Both of these approaches require a large number of rays-per cell to accurately obtain the intensity, $\mathcal{O}(100)$ \cite{debayle_unified_2019}.
...
The mesh used for hydrodynamic calculations is often not well suited for ray tracing calculations.

HERE TALK GENERICALLY ABOUT SCALE LENGTHS ETC., OVERCRITICAL CELLS
...
The optimal domain balancing minimises communication between ranks, which leads to cubic\footnote{Cubic in number of cells, not necessarily in physical space for non-Cartesian geometries.} subdomains.
...
When required for spherical and cylindrical simulations, \textsc{Solas} therefore takes the hdyrodynamic variables on the \textsc{Chimera} grid and re-domain balances the grid for the raytrace, such that the splitting does not occur in the radial coordinate, as is shown in figure MAKE FIGURE HERE!!!

Note that a domain balanced, \ac{MPI} communication approach has been employed for the \textsc{Solas} grid and ray transfer.
An alternative approach to parallelization of the module would be to use the \ac{OpenMP} package, where processors share memory across a computational node.
