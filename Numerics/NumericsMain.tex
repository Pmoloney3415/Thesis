...
Codes that use this formulation typically use a damping value $\nu_{\text{ia}}=0.2$ for CH plasmas, which is the value used for all fluid gain simulations in this chapter.
...
These measures significantly reduces memory overhead for multi-dimensional \ac{CBET} simulations, however high resolution 3-D simulations are still extremely memory intensive, which is an issue also experienced by \textsc{Ifriit}, the other 3-D \ac{CBET} code coupled to hydrodynamics.
...
These results are from a 2 beam caustic test problem, presented in Sec.~\ref{sec:SOLAS_CBET_caustic_test}, but the specifics of the problem are not important for the discussion here, simply that \ac{CBET} in the presence of a caustic occurs, meaning that a caustic region exists where $F_{\text{CP}}$ will lead to modified gains which affect energy conservation.
