...
If using nearest neighbour interpolation, which is used in \textsc{Solas} for interpolation of all field quantities onto rays, this means a resolution of $\Delta x\ll\lambda_0$ must be used, which is a much higher resolution than is required for ray-tracing and \ac{CBET} without the \ac{EI}.
...
In this section several test problems will be presented that compare electric fields obtained using \textsc{Solas} field solver in the absence of \ac{CBET}, to those from the wave based solver \textsc{Lpse} in direct-drive relevent plasma profiles.
...
\begin{figure}[t!]
    \includegraphics[width=1.0\linewidth]{Numerics/Images/2D_field_reconstruction_lineouts.png}
    \centering
...
The simulations presented here used a scale factor of $S=1/64$ with simulation bounds $x\in [-20,20]\ \mu\text{m}$ and $y\in [-20,20]\ \mu\text{m}$ and resolution $\Delta x\ \sim 20 \text{nm}$.
A single beam propagated parallel to $+\hat{y}$ and was centred on $x=0\ \mu\text{m}$.
