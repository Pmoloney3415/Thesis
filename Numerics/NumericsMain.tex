...
\ac{LPIs} cannot be included in the simple framework described above because they are non-linear\footnote{Non-linear in this context means that the interaction involves multiple light and plasma waves.} and reduced theoretical models of the interaction rely on knowledge of the electric field or intensity of the light~\cite{randall_theory_1981}.
...
Sec.~\ref{sec:SOLAS_field_reconstruc} describes the approach taken to solving for the amplitude of the rays in \textsc{Solas}, which is to track the area of a triangle\footnote{For a 2D ray trace, a co-planar, pair of rays is used rather than a triangle.} of rays around a standard \ac{GO} ray.
...
In some computational direct-drive studies, particularly in 1D \ac{Rad-Hydro} simulations, simplified laser models are employed in which rays travel radially toward the target~\cite{paddock_one-dimensional_2021}.
...
A non-exhaustive list of existing codes will be presented here, provided as context for the work presented in this chapter.
...
\textsc{Ifriit} has been coupled to the 3-D \ac{Rad-Hydro} code, \textsc{Aster} from \ac{LLE}, and prior to the development of \textsc{Solas} and its coupling to the \textsc{Chimera} \ac{Rad-Hydro} code, \textsc{Aster}-\textsc{Ifriit} was the only code combination capable of conducting 3-D direct-drive simulations with in-line \ac{CBET}~\cite{colaitis_inverse_2021}.
...
This approach was coupled to the \textsc{Chic} 2-D \ac{Rad-Hydro} code, but the approach is difficult to extend to 3-D as the implementation relied on interacting only rays whose centroids crossed, which will not occur in 3-D for non-coplanar rays~\cite{colaitis_modeling_2015}.
...
Sec.~\ref{sec:SOLAS_field_reconstruc} describes the implementation of this approach into the \textsc{Chimera} 3D \ac{Rad-MHD} code.
A similar approach, simplified by assuming uniform illumination of a direct-drive target, has been implemented into the \textsc{Lilac} \ac{Rad-Hydro} code~\cite{igumenshchev_crossed-beam_2010, igumenshchev_crossed-beam_2012}.
It had previously however, not been coupled to a multi-dimensional \ac{Rad-Hydro} code.
...
This section will describe the approach taken in the \textsc{Solas} code to create a grid structure which is well suited to direct-drive simulations.
This grid has been found to accurately resolve laser energy deposition both with and without \ac{CBET} in a memory-efficient manner.
...
If computing a laser raytrace through this domain for a typical direct-drive calculation however, initially rays travel approximately radially and therefore regularly encounter processor boundaries where they must be transferred between ranks.
A more optimal domain decomposition for the raytrace minimises radial splitting to avoid excessive passing of rays.
For spherical and cylindrical simulations, \textsc{Solas} therefore takes the hydrodynamic variables on the \textsc{Chimera} grid and re-domain balances the grid for the raytrace, such that the splitting does not occur in the radial coordinate.
...
While this is certainly a preferable approach for a standard raytrace to the \ac{MPI} procedure described above, including a model for \ac{CBET} is more challenging.
...
Therefore, using multiple computing nodes is often a necessity for 3D \ac{CBET} calculations and \ac{OpenMP}-\ac{MPI} hybrid approaches are required, deemed too significant an undertaking for the scope of the work presented in this thesis.
...
This limits the hydrodynamic timestep, because stable explicit timesteps are inversely proportional to edge lengths, increasing the cost of 3D spherical simulations.
...
Computing \ac{CBET} however requires at least a single ray from each interacting beam to pass through each computational grid cell where the interaction should be important.
...
The problems are closely related as a higher resolution grid will require a larger density of rays to get equivalent ray-per-cell statistics.
...
In this method of ray initialisation, demonstrated in Fig.~\ref{fig:SOLAS_ray_init}.c, each beam is assigned a number of rays and a maximum ray width\footnote{The maximum initialisation width is usually set to be the radius at which $I=I_0e^{-3}$.}, beyond which rays are not initialised.
...
The error scaling in dictated by the error on the lowest order interpolation used in the algorithm, which is the linear interpolation of $n_e$ for the default adaptive RK45 solver, which leads to the $N_r^{-1}$ scaling.
...
The heatfront $x_{\text{analytic}}(t)$, defined as the largest $x$ coordinate at a time, $t$ where $T_e>T_{e0}$, has an analytic solution, derived by Denavit and Phillion~\cite{denavit_laser_1994},
\begin{equation}
...
Firstly, because the amplitude diverges, the field values can become very large and therefore must be capped to diffraction limited values (the \textsc{Solas} methods for this are discussed in more detail in Sec.~\ref{sec:SOLAS_caustic_cap}).
Power change of rays due to \ac{CBET} scale exponentially with field strength squared, so erroneously large field values are extremely problematic to accurately computing power changes.
...
If using nearest neighbour interpolation, such as in \textsc{Solas} for interpolation of all field quantities onto rays, this requires a grid resolution of $\Delta\ll\lambda_0$ must be used, which is a much higher resolution than is required for ray-tracing and \ac{CBET} without the \ac{EI}.
...
For this problem, 1-D Cartesian geometry is used, so the density profile is obtained by transforming the spatial coordinate from equation~\ref{eq:SOLAS_test_ne}, $r\rightarrow x-36.42\ \mu\text{m}$.
For this problem, the beam is travelling in the $+\hat{x}$ direction, with $I_0 = 14\ \text{W}/\text{cm}^{2}$.
...
A high resolution is used here in order to resolve the highly oscillatory coherent field sum and also enables use of the \ac{EI} field capping method.
...
For light travelling normally up density gradient, the critical density is the location of the caustic, because the ray area, $S(\tau)$, is constant and therefore the amplitude diverges when $\varepsilon\rightarrow 0$.
...
The initial `field reconstruction' raytrace in the absence of \ac{CBET}, follows the procedure described in Sec.~\ref{sec:SOLAS_field_reconstruc} to obtain the field for every sheet on the \textsc{Solas} mesh.
...
Unlike the fluid gain, this thoery correctly captures the resonance and has no effective free parameters, making it the judicious choice for predictive simulations.
...
\begin{figure}[t!]
    ...
    \centering
    \caption{A series of figures which illustrate how the caustic gain truncation and caustic region identification algorithms in \textsc{Solas} work.
...
Near caustics, rays turn sharply and fields have a very sharp \textit{cut off} as be seen in the lineouts from Fig.~\ref{fig:SOLAS_2d_field_test}.
...
If a ray from another beam were present with $14.3 \lesssim y < 15\ \mu\text{m}$, nearest neighbour interpolation would lead to ray undergoing a \ac{CBET} interaction in this \textit{unlit} region, where no rays from the pump sheet are present.
...
The \textsc{Solas} model is however simpler to implement and does act to effectively increase the resolution of the cells near caustics.
...
\end{equation}
which should be applied in the caustic region, which is defined in Eq.~\ref{eq:SOLAS_causticregion}, as the region where $|\varphi_1-\varphi_2|\leq\lambda_0/2$.
...
The geometric approach that has been developed to estimate when a ray is in the caustic region of another beam, ($|\varphi_1-\varphi_2|\leq\lambda_0/2$), shall now be outlined.
...
If there are errors in, for example the field reconstruction algorithm, then the pump depletion iterations will return a steady state solution, however this is not guaranteed to be accurate, as would be obtained by a higher fidelity solver.
...
As in the \textsc{BeamCrosser} code, the assumption in \textsc{Solas} is made that minimising the $\Delta E$ by altering the \ac{CBET} gains in the caustic region will bring the result into better agreement with the true solution~\cite{follett_validation_2022}.
...
When the \ac{CBET} model runs in-line with the hydrodynamics, the value of $f_{\text{CP}}$ from the previous timestep when \ac{CBET} was calculated is used which means that energy conservation iterations are frequently not necessary.
...
This section shall present results from several \ac{CBET} test problems where the results from \textsc{Solas} are compared to the wave based solver \textsc{Lpse}.
...
Beams in the simulations all have a vacuum wavelength, $\lambda_0=0.351\ \text{nm}$ and a super-gaussian spot profile, which was defined in Eq.~\ref{eq:supgaus}.
For consistency with \textsc{Lpse}, the Doppler shift of laser frequency is not calculated in these simulations so its effect on \ac{CBET} is neglected, \textit{i.e.} the right-hand side of Eq.~\ref{eq:SOLAS_doppler_used} is forced to be zero.
...
The energy conservation error after the pump depletion iterations was very small, $\Delta E \sim 10^{-6}$, as expected because the \ac{CBET} interaction does not involve any caustics.
...
The angled-line features in both plots are from the caustic region of the beam, where 
...
Ray-traces were conducted every hydrodynamic timestep, without computing \ac{CBET} up to $150\ \text{ps}$\footnote{Note that the very low intensity from this section of the pulse means that the \ac{CBET} gains are negligible.} after which a full raytrace routine, including the effect of \ac{CBET} was conducted every $10\ \text{ps}$.
...
Parameter scans of the raytrace frequency confirmed that the behaviour of \textsc{Omega} simulations is well converged with this frequency.
...
Including \ac{CBET} in the \textsc{Chimera}-\textsc{Solas} simulation gives much better agreement across all metrics, which is primarily due to correctly capturing the reduction in absorption.
...
DT has a significantly lower ionisation state compared to the ablator and therefore the electron-ion collision frequency decreases, corresponding to a drop in the \ac{inv-Brem} absorbtion and therefore an increase in the reflected light fraction.
...
At these times, caustic \ac{CBET} interactions are very important and therefore $\Delta E$ is very sensitive to the value of $f_{\text{CP}}$.
...
\begin{figure}[t!]
    ...
    \centering
    \caption{Plotted in this figure are \ac{CBET} deposition diagnostics from $t=1.6\ \text{ns}$ into the \textsc{Chimera}-\textsc{Solas} simulation of \textsc{Omega} 89224, with \ac{CBET}.
...
Note that the no-\ac{CBET} profile plotted here is not from the no-\ac{CBET} simulation of \textsc{Omega} 89224, but the power deposition from the field reconstruction raytrace of the \ac{CBET} simulation, \textit{i.e.}, the deposition through the same hydrodynamic profiles, but without the effect of \ac{CBET} on the power deposition.
At this time, \textsc{Solas} estimate that \ac{CBET} reduces the deposition versus by about 40 \% compared to the no-\ac{CBET} deposition.
...
Very good agreement is again observed between \textsc{Solas}, \textsc{Ifriit} and \textsc{BeamCrosser} across the spectrum.
...
\textsc{Solas} appears to slightly overestimate the modes 9 and 11, which have negligable values for both \textsc{Ifriit} and \textsc{BeamCrosser}.
...
\begin{figure}[t!]
    ...
    \centering
    \caption{Ploptted in this figure are the 3-D electric fields from the 3-D post-process.
...
The incident and reflected sheet fields\footnote{Note that icident and reflected here refer to which sheet the ray belongs to, \textit{i.e.} if it is before or after the caustic location.} for a single beam both with and without \ac{CBET} are plotted in Fig.~\ref{fig:SOLAS_IFRIIT_test_fields}.
the incident fields without and with \ac{CBET} are plotted in Fig.~\ref{fig:SOLAS_IFRIIT_test_fields}.a \& Fig.~\ref{fig:SOLAS_IFRIIT_test_fields}.c respectively, and share the left colorbar.
...
Fig.~\ref{fig:SOLAS_IFRIIT_test_fields}.b \& Fig.~\ref{fig:SOLAS_IFRIIT_test_fields}.d plot the reflected fields without and with \ac{CBET} respectively, and share the right colorbar.
...
Firstly, peak absorbtion occurs at the maximum radial penetration depth of the light into plasma, which is typically close to where rays experience their caustic and change sheet.
...
Presented in this chapter was the implementation and validation of a 3-D raytrace and \ac{CBET} model, \textsc{Solas}, for the 3-D \ac{Rad-Hydro} code \textsc{Chimera}.
Prior to this work, a simple 1-D raytrace was used to model the laser for direct-drive implosions which was strongly detrimental to the ability of the code to accurately model these experiments.
...
These factors severly limited the predictive capability of the code to model these experiments.
...
The 3-D laser raytrace enables \textsc{Chimera} to model direct-drive implosions and other laser-driven \ac{HEDP} experiments.
It is also a crucial pre-requiste to a ray-based \ac{CBET} model.
