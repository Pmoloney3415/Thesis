...
The simulation platform described in this section could prove to be a promising computational design tool to help to understand how to optimise zooming to mitiagte \ac{CBET}, without introducing overly detrimental beam-mode degradation.
...
The physics of the implosion is however different in clyindrical as opposed to spherical geometry.
...
Cylindrical implosion experiments are also frequenctly conducted on laser facilities which explcitily relate their work to spherical implosions~\cite{perez-callejo_cylindrical_2022,tubbs_direct-drive_1999}.
Although the differences between the two regimes are not quantified in the work conducted in this chapter, future work could be done to extend the platform to a 2-D `plane' in spherical rather than cyllindrical geometry, which would capture the spherical convergence of the target and divergence of the coronal flow.
...
The initial conditions are symmetric about the azimuth and in the plane normal to te $\hat{\vec{z}}$ direction.
...
The maximum power of the pulse for each 2-D simulation was tuned from a seperate set of 1-D simulations, such that the bangtime occured at 2.5 ns.
...
The difference between implosions therefore was therefore primarily due to differences in the spatial location of the deposited power.
If the incident energy were fixed, increasing $R_b/R_t$ would lead to more \ac{CBET}, which would result in less energy couped to the target.
...
1-D calucaltions showed that the effect of including radiation transport was relatively small, reducing the bangtime by $\sim 0.1\ \text{ns}$, primarily due to temperature losses in the corona.
...
This was done via a series of 1-D, with-\ac{CBET} simulations which varied the maximum power of the square pulse, $P_{\text{max}}$, for each $R_b/R_t$ value to obtain an implosion with $t_{\text{bang}}=2.50 \pm 0.01\ \text{ns}$.
...
This means that the no-\ac{CBET} implosions have a slightly increased drive efficiency compared to \ac{CBET} implosions, because thermal conduction does not have to transport energy as far from the absrption region to the ablation surface.

Tab.~\ref{tab:res1_1d_tuning} shows implosion metrics from all of the tuned 1-D implosions.
As can be seen, the incident maximum power for the \ac{CBET} simulations, $P_{\text{max}}$ increases with increasing $R_b/R_t$ because larger $R_b/R_t$ leads to more \ac{CBET} and therefore less absorption, so more incident power is required to maintain the same absorbed energy.
The maximum intensity of each beam at peak power, $I_0$ is non-monotomic, because although the maximum power increases, the beam radius also increases, which limits the increase in maximum intensity.
...
This is because more \ac{CBET} occurs for the larger $R_b/R_t$ simulations and therefore the difference in deposition radius increases between \ac{CBET} and no-\ac{CBET} simulations, marginally improving the effective drive efficency of the no-\ac{CBET} results.
...
\textit{Streak plots}, which plot time resolved hydrodynamic quantites as a function of radius and time, of $n_e$ and $T_e$ are plotted in Fig.~\ref{fig:Res1_streaks} for with-\ac{CBET} simulations at two separate $R_b/R_t$ values.
