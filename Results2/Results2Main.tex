...
Experimental data from these magnetised exploding pushers, demonstrated a clear amplification of the mode-2 due to magnetisastion, which could affect \ac{CBET} scattering.
The beam configuration, capsule initial conditions and pusle shape are shown in Fig.~\ref{fig:Res2_simconfig}.
...
40 beams from the \textsc{Omega} laser, delviered a total of $17.7\ \text{kJ}$ laser energy to a $2.5\ \mu\text{m}$ thick, glass ($\text{SiO}_2$\footnote{By ion number density, the material was comprised of $1/3$ $\text{Si}$ and $2/3$ $\text{O}$.}) capsule, filled with room temperature and pressure $\text{D}_2$.
...
1-D simulations were ran from beginning to end in spherical with $r\in[0,1400]\ \mu\text{m}$.
2-D, $r,\theta$ calculations assumed azimuthal symmetry of the hydrodyanmics and conducted an initial `drive-phase' in spherical coordinates, with $r\in[80,1400]\ \mu\text{m}$, where the central cutout region was removed to avoid taking small radiation transport timesteps due to the small cell faces close to the axis.
The 2-D drive-phase grid used 120 cells in the polar direction.
When the shock reached this cutout region, hydrodynamic variables wer trilinearly interpolated onto a 2-D cylindrical $r_{\text{cyl}},z$ mesh with fixed resolution, $\Delta_{r,\text{cyl}}=\Delta_z=1\ \mu\text{m}$, and $r_{\text{cyl}}\in[0,1200]\ \mu\text{m}$ and $z\in[-1200,1200]\ \mu\text{m}$, for the `stagnation-phase'.
...
Explicitly, these simulations conducted a no-\ac{CBET} ray-trace, where the power of all rays was noramlised to unity.
...
This created a hydrodynamically similar implosion to the \ac{CBET} ray-trace, and via comparison of these two simualtions, the impact of \ac{CBET} relocation-of-deposition upon the hydrodyanmics was studied.
Temporally-and-spatially integrated results from all simulations are presented in Tab.~\ref{tab:Res2_magexpl_results} and are referred to when relevent throughout the chapter.
